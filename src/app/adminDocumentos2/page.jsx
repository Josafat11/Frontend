...
function TermsConditionsPage() {
  ...
  const [newTerms, setNewTerms] = useState({
    ...
    content: "",
    effectiveDate: "",
  }); // Para los nuevos términos
  const router = useRouter();

  // Definición de la función fetchTermsConditions antes de su uso
  const fetchTermsConditions = async () => {
    const token = localStorage.getItem("token"); // Obtén el token del almacenamiento local
    console.log(token);
    const response = await fetch(
      `${CONFIGURACIONES.BASEURL3}/docs/terms-conditions/current`,
      {
        method: "GET",
        headers: {
  ...
  const handleCreateTerms = async () => {
    try {
      const token = localStorage.getItem("token"); // Obtén el token del almacenamiento local
      const response = await fetch(
        `${CONFIGURACIONES.BASEURL3}/docs/terms-conditions`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        console.error(
          "Error al crear los términos y condiciones:",
          response.statusText
        );
      }
    } catch (error) {
  ...
  if (loading) {
    return (
      <p className="text-center mt-20">Cargando términos y condiciones...</p>
    );
  }
